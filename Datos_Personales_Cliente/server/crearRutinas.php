...
function enviarEjercicio($data)
{
    ...
    $sql2 = "INSERT INTO rutinas (fecha_ruti, usuario_idusuario) VALUES ('$fecha_ruti', '$idusuario')";
      
    if (($result = mysqli_query($conex, $sql2)) === false) {
        die(mysqli_error($conex));
    } 

    $sql1 = "SELECT MAX(idRutina) AS idRutina FROM rutinas";
    ...
    for ($i=0; $i < count($arrayEjercicios); $i++) {         
        ...
        $sql = "INSERT INTO ejerciciosrutina (Series, Repeticiones, idEjercicios, ejercicios_idEjercicios) VALUES ('$Series', '$Repeticiones', '$idEjercicios', '$idEjerciciosRutina')";
