...
$(function() {
    ...
    $('#tbl-rutinas tbody').on('click', '.btnEliminar', function(){
...
function cargar_select_ejercicios(data){
    ...
    var option = '';
    for (var i = 0; i < data.length; i++) {
        option += '<option value="' + data[i]['idEjercicios'] + '">' + data[i]['Nombre'] + '</option>';
    }
    for (let i = 0; i <= contadorFilas; i++) {
...
function enviarEjercicio() {
    ...
    arrayEjercicios = [];
    $('#tbl-rutinas tbody tr').each(function() {
...
function nuvaFila(data){
    ...
    var filas = `<tr>
                    ...
                    <td><input id="Series" class="input is-small" type="number"  min="1" max="6" placeholder="Cantidad"></td>
                    <td><input id="Repeticiones" class="input is-small " type="number" min="1" max="50"  placeholder="Cantidad"></td>
                    <td> <button type="button" id="btnEliminar${contadorFilas}" class="button is-warning is-rounded btnEliminar"><b>Eliminar</b></button></td>
                </tr>`;
    $('#tbl-rutinas').append(filas);
}